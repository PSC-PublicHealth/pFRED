...
void DefaultIHM :: setup(Disease *disease)
{
  IHM::setup(disease);
...
Trajectory * DefaultIHM :: getTrajectory(Infection *infection, std :: map<int, double> *loads)
{
  // TODO  take loads into account - multiple strains
	...
	if (sequential) { // SEiIR model
		...
		if (will_be_symptomatic) { days_symptomatic = get_days_symp(); }
	} 
	else { // SEIR/SEiR model
		if (will_be_symptomatic) { days_symptomatic = get_days_symp(); }
		else { days_asymptomatic = get_days_asymp(); }
	}
