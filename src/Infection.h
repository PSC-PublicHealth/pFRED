...
class Infection {
  public:
    ...
    // general
    UNIT_TEST_VIRTUAL Disease *get_disease() const {
      return disease;
      }
    UNIT_TEST_VIRTUAL Person *get_infector() const {
      return infector;
      }
    UNIT_TEST_VIRTUAL Place *get_infected_place() const {
      return place;
      }
    UNIT_TEST_VIRTUAL int get_infectee_count() const {
      return infectee_count;
      }
    UNIT_TEST_VIRTUAL int add_infectee() {
      return ++infectee_count;
      }
    ...
    UNIT_TEST_VIRTUAL int get_infectious_date() const {
      return infectious_date - offset;
      }
    UNIT_TEST_VIRTUAL int get_symptomatic_date() const {
      return symptomatic_date - offset;
      }
    int get_asymptomatic_date() const {
      return asymptomatic_date - offset;
      }
    UNIT_TEST_VIRTUAL int get_recovery_date() const {
      return recovery_date - offset;
      }
    ...
    UNIT_TEST_VIRTUAL bool is_symptomatic() {
      return symptoms > trajectory_symptomaticity_threshold;
      }
    UNIT_TEST_VIRTUAL double get_susceptibility() const {
      return susceptibility;
      }

    UNIT_TEST_VIRTUAL double get_symptoms() const {
      return symptoms;
      }
    UNIT_TEST_VIRTUAL void modify_develops_symptoms(bool symptoms, int cur_day);
    UNIT_TEST_VIRTUAL void modify_susceptibility(double multp) {
      susceptibility *= multp;
      }
    UNIT_TEST_VIRTUAL void modify_infectivity(double multp) {
