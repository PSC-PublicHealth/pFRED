...
class Vaccine_Dose {
  ...
  public:
    ...
    //Parameter Access
    UNIT_TEST_VIRTUAL Age_Map* get_efficacy_map()       const {
      return efficacy;
      }
    UNIT_TEST_VIRTUAL Age_Map* get_efficacy_delay_map() const {
      return efficacy_delay;
      }

    UNIT_TEST_VIRTUAL double  get_efficacy(int age)         const {
      return efficacy->find_value(age);
      }
    UNIT_TEST_VIRTUAL double  get_efficacy_delay(int age)   const {
      return efficacy_delay->find_value(age);
      }
    UNIT_TEST_VIRTUAL int     get_days_between_doses()  const {
