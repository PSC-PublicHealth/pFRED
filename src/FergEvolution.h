...
struct FergEvolution_Report : public Transaction {
  ...
  struct Extended_Daily {
    ...
    void add( Window * window, int cell, int offset, int length ) {
      for ( int d = offset; d < offset + length; ++d ) {
        assert( cell < num_clusters );
        assert( d < window_size );
        ++( window[ cell ][ d ] );
      }
    }

    void add_exposed( int cell, int offset, int length ) {
      if ( offset < window_size ) {
        add( exposed, cell, offset, offset + length < window_size ? length : window_size - offset );
      }
      if ( offset + length >= window_size ) {
        add( next_exposed, cell, 0, ( 2 * window_size ) - ( offset + length ) );
      }
    }

    void add_prevalence( int cell, int offset, int length ) {
      if ( offset < window_size ) {
        add( prevalence, cell, offset, offset + length < window_size ? length : window_size - offset );
      }
      if ( offset + length >= window_size ) {
        add( next_prevalence, cell, 0, ( 2 * window_size ) - ( offset + length ) );
      }
    }
  ...
  int num_clusters;
  // sum of prevalence for all cells
  int total_activity;
  // <----------------------------------------------------------------------- Constructor
  ...
  // Report case
  void report_incidence( int infector_cell, int infectee_cell, int strain ) {
    // if this strain isn't in the daily map, create entries for it
    if ( daily_strain_map.find( strain ) == daily_strain_map.end() ) {
      daily_strain_map[ strain ].init( num_clusters );
    }
    ...
    if ( statement_number < n_stmts ) {
      if ( statement_number == 0 ) {
        return "create table if not exists strain_incidence \
            ( day integer, cell integer, strain integer, incidence integer );";
      }
      if ( statement_number == 1 ) {
        return "create table if not exists novel_mutations \
            ( day integer, cell integer, parent_strain integer, child_strain integer );";
      }
      ...
      if ( statement_number == 3 ) {
        return "create table if not exists strain_prevalence \
            ( day integer, cell integer, strain integer, prevalence integer );";
      }
      if ( statement_number == 4 ) {
        return "create table if not exists strain_exposed \
            ( day integer, cell integer, strain integer, exposed integer );";
      }
    }
    else {
...
class FergEvolution : public MSEvolution {
 ...
 public:
  ...
  void get_infecting_and_infected_cells( Infection * infection,
 ...
 protected:
  ...
  // thread-safe state array of Person pointers per cell 
