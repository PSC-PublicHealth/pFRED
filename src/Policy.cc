...
bool Policy::choose_first_positive(Person* person, int strain, int current_day){
  for(unsigned int i=0; i < decision_list.size(); i++){    
    if(decision_list[i]->evaluate(person,strain,current_day) == 1) return true;
  }
  return false;
}   

bool Policy::choose_first_negative(Person* person, int strain, int current_day){
  for(unsigned int i=0; i < decision_list.size(); i++){    
    if(decision_list[i]->evaluate(person,strain,current_day) == -1) return false;
  }
  return true;
}
