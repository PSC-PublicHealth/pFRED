...
public:
  ...
  void clear();
	...
	void set_num_equations(int neq) { num_equations = neq; }
	void set_num_strains(int ns) { num_strains = ns; }
	void set_duration(int dur) { duration = dur; }
	void set_absolute_tolerance(double abstol);
	void set_relative_tolerance(double reltol) { relative_tolerance = reltol; }
	void set_max_steps(double s) { max_steps = s; }
	void set_time_scale(int t) { time_scale = t; }
  int get_time_scale () { return time_scale; }

    //Getters/setters for constants
	void set_g_v   (double val_g_v, int strain )  { g_v[strain]   = val_g_v  ; }
	void set_g_vh  (double val_g_vh, int strain)  { g_vh[strain]  = val_g_vh ; }
	void set_g_hv  (double val_g_hv, int strain)  { g_hv[strain]  = val_g_hv ; }
	void set_a_v   (double val_a_v )  { a_v   = val_a_v  ; }
	void set_a_i   (double val_a_i )  { a_i   = val_a_i  ; }
	double get_g_v   (int strain) { return g_v[strain]   ; }
	double get_g_vh  (int strain) { return g_vh[strain]  ; }
	double get_g_hv  (int strain) { return g_hv[strain]  ; }
	double get_a_v   () { return a_v   ; }
    ...
    void set_I (double val_I, int s) { initial_values_[I + s * offset] = val_I; }
    void set_P (double val_P, int s) { initial_values_[P + s * offset] = val_P; }
    void set_A (double val_A, int s) { initial_values_[A + s * offset] = val_A; }
    void set_S (double val_S, int s) { initial_values_[S + s * offset] = val_S; }
    
    void set_H (double val_H) { initial_values_[H] = val_H; }
    void set_M (double val_M) { initial_values_[M] = val_M; }
    void set_F (double val_F) { initial_values_[F] = val_F; }
    void set_R (double val_R) { initial_values_[R] = val_R; }
    void set_E (double val_E) { initial_values_[E] = val_E; }

    double get_V (int s) { return initial_values_[V + s * offset]; }
    double get_I (int s) { return initial_values_[I + s * offset]; }
    double get_P (int s) { return initial_values_[P + s * offset]; }
    double get_A (int s) { return initial_values_[A + s * offset]; }
    double get_S (int s) { return initial_values_[S + s * offset]; }
    
    double get_H () { return initial_values_[H]; }
    double get_M () { return initial_values_[M]; }
    double get_F () { return initial_values_[F]; }
    double get_R () { return initial_values_[R]; }
    double get_E () { return initial_values_[E]; }


	int get_duration() { return duration; }
	int get_num_equations() { return num_equations; }
	int get_num_strains() { return num_strains; }
  ...
  double get_datum_avg(double start, double end, int steps, int equation_number);
	...
	double * get_equation_data(int equation_number) { return get_equation_data(equation_number, duration); }
	double * get_equation_data(int equation_number, int days);

	double * get_viral_titer_data(int s)    { return get_equation_data(V + offset * s); }
	double * get_infected_cells_data(int s) { return get_equation_data(I + offset * s); }
	double * get_interferon_data()     { return get_equation_data(F); }
