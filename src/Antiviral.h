...
class Antiviral {
  ...
  public:
    ...
    UNIT_TEST_VIRTUAL double  get_reduce_infectivity()    const {
      return reduce_infectivity;
      }
    UNIT_TEST_VIRTUAL double  get_reduce_susceptibility() const {
      return reduce_susceptibility;
      }
    UNIT_TEST_VIRTUAL double  get_reduce_asymp_period()   const {
      return reduce_asymp_period;
      }
    UNIT_TEST_VIRTUAL double  get_reduce_symp_period()    const {
      return reduce_symp_period;
      }
    UNIT_TEST_VIRTUAL double  get_prob_symptoms()         const {
      return prob_symptoms;
      }
    UNIT_TEST_VIRTUAL int     get_course_length()         const {
      return course_length;
      }
    UNIT_TEST_VIRTUAL double  get_percent_symptomatics()  const {
      return percent_symptomatics;
      }
    UNIT_TEST_VIRTUAL double  get_efficacy()              const {
      return efficacy;
      }
    UNIT_TEST_VIRTUAL int     get_start_day()             const {
      return start_day;
      }
    UNIT_TEST_VIRTUAL bool    is_prophylaxis()            const {
      return prophylaxis;
      }
    ...
    // Logistics Functions
    UNIT_TEST_VIRTUAL int get_initial_stock()         const {
      return initial_stock;
      }
    UNIT_TEST_VIRTUAL int get_total_avail()           const {
      return total_avail;
      }
    UNIT_TEST_VIRTUAL int get_current_reserve()       const {
      return reserve;
      }
    UNIT_TEST_VIRTUAL int get_current_stock()         const {
      return stock;
      }
    UNIT_TEST_VIRTUAL int get_additional_per_day()    const {
    ...
    UNIT_TEST_VIRTUAL void set_policy(Policy* p)                {
      policy = p;
      }
    UNIT_TEST_VIRTUAL Policy* get_policy() const                {
      return policy;
      }

    // To Be depricated
    UNIT_TEST_VIRTUAL void add_given_out(int amount)            {
      given_out+=amount;
      }
    UNIT_TEST_VIRTUAL void add_ineff_given_out(int amount)      {
